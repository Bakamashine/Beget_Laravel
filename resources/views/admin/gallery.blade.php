...
@section('content')
    ...
    <div class="mt-6 bg-white h-[40px] border-[1px] mobile:w-full flex max-mobile:flex-wrap max-mobile:h-auto  max-mobile:w-4/5">

            <div class="h-full w-32 mr-4">
                <form method="get" action="{{route("gallery")}}" class="my-auto h-full flex justify-center items-center max-mobile:my-4">
    ...
    {{-- Добавление файлов --}}
    <div class="mt-4 bg-white  border-[1px] mobile:w-full max-mobile:w-4/5">
    ...
    19.02.25 Похоже что всё-таки нашёл нейросеть (от сбера), но разбираться в этом времени уже не было.
    Многие изображения генерировал с помощью нейросети BlackBox Ai, но у неё закрытое API (или за него нужно платить 40 долларов, так и не понял)
    --}}
        ...
        @if (count($files) > 0) 
            <div class="flex flex-wrap justify-center">
                @foreach($files as $file)
                    <div class="mr-3">
                        <img key="{{$file->id}}" src="{{$file->path}}" class="mobile:w-[500px] mobile:h-[400px] m-3 rounded-md max-mobile:w-4/5">
                        <p class="text-center italic  max-mobile:w-4/5 max-mobile:text-xs">{{$file->name}}</p>
                    </div>
                @endforeach
            </div>
        @else
    ...
    <div class="mt-10">
        ...
        @if (count($files) > 0) 
            <div class="flex flex-wrap justify-center">
                @foreach($files as $file)
                    <video controls class="w-[500px] h-[500px] mr-10 object-cover">
