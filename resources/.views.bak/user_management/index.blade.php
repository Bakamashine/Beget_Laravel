...
@section('content')
    ...
    <p class="italic">На этой странице вы можете добавлять, удалять и всячески редактировать пользователей</p>
	...
	<form method="post" class="space-y-4" action="{{route("user_management.store")}}">
		@csrf
            ...
            <div>
                ...
                <input type="text" id="password" name="password" class="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                @error('password')
                    {{$message}}
                @enderror
            </div>
